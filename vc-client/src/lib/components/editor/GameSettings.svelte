<script lang="ts">
    ...
    const goals = [
        ...
        {name:'Target Square', desc:"Move one of your selected pieces to a target square on the board"}
        ];
   ...
   <h2 class="font-bold md:text-xl sm:text-lg">Select Objective:</h2>
    <div class="flex flex-col">
        ...
        <div
            ...
            {selectedGoalIdx === index ? 'border-indigo-900 border-solid border-2 rounded' : 'border-gray-300'}"
            on:click={() => selectGoal(goal,index)}
        >
            <input type="radio" class="form-radio text-indigo-600 h-4 w-4" value={goal.name} checked={selectedGoalIdx === index} />
