			...
			<div class="flex justify-between">
				...
				{:else}
					<div class="text-white md:flex items-center justify-end relative">
						...
						<button class="bg-transparent text-white py-2 px-4 md:text-lg text-sm hover:bg-gray-800 cursor-pointer">
							{username} <i class="fa-solid fa-user" style="color: #feffff;"></i>
						</button>
						{#if isOpen}
						<ul class="dropdown-menu absolute pt-2 right-0">
							<li><a href="/profile" class="block px-4 py-2 hover:bg-gray-800" on:click={closeDropdown}>Profile</a></li>
							<li><a href="/logout" class="block px-4 py-2 hover:bg-gray-800" on:click={closeDropdown}>Logout</a></li>
						</ul>
						{/if}
						</div>
					</div>
