...
func TestParseEnPassant(t *testing.T) {
	require.Equal(t, FileRankToIndex(4, 2, 8), ParseEnPassant("e3", 8))
	require.Equal(t, FileRankToIndex(3, 5, 8), ParseEnPassant("d6", 8))
	require.Equal(t, -1, ParseEnPassant("-", 8))
	require.Equal(t, -1, ParseEnPassant("z9", 8))
}
...
func TestSquare(t *testing.T) {
    tests := []struct {
        ...
        {"Valid A1 (8x8)", Position{Files: 8, Ranks: 8}, "a1", 0, nil},
        {"Valid H8 (8x8)", Position{Files: 8, Ranks: 8}, "h8", 63, nil},
        {"Valid E4 (8x8)", Position{Files: 8, Ranks: 8}, "e4", 28, nil},

        // Custom Board Sizes
        {"Valid C3 (5x5)", Position{Files: 5, Ranks: 5}, "c3", 12, nil},
        {"Valid J10 (10x12)", Position{Files: 10, Ranks: 12}, "j10", 99, nil},
